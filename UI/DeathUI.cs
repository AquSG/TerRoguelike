...
namespace TerRoguelike.UI
{
    public static class DeathUI
    {
        ...
        {
            int deadTime = TerRoguelike.mpClient ? TerRoguelikePlayer.allDeadTime : modPlayer.deadTime;
            float opacity = MathHelper.Clamp(MathHelper.Lerp(0, 1f, (deadTime - 120) / 60f), 0, 1f);
            ...
            if (TerRoguelikeWorld.currentLoop > 0)
            {
                ...
                ChatManager.DrawColorCodedStringWithShadow(spriteBatch, deathFont, loopString, moonDrawPos + new Vector2(75, 12), Color.Lerp(Color.Cyan, Color.Blue, 0.15f) * opacity, 0, Vector2.Zero, new Vector2(0.6f));
            }
