...
namespace TerRoguelike.NPCs.Enemy
{
    public class IchorSticker : BaseRoguelikeNPC
    {
        ...
        public override void AI()
        {
            ...
            NPC.rotation = MathHelper.PiOver2 * Math.Abs(NPC.velocity.X) * 0.16f * (NPC.velocity.X == 0 ? 0 : ((NPC.velocity.X) / Math.Abs(NPC.velocity.X)));
            modNPC.RogueFlyingShooterAI(NPC, 1f, 1f, 0.02f, 160f, 480f, attackTelegraph, attackCooldown, ModContent.ProjectileType<IchorBlob>(), 8f, new Vector2(0, 10).RotatedBy(NPC.rotation), NPC.damage, true, 0.98f);
