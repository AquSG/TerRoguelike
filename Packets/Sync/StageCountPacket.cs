...
{
    public sealed class StageCountPacket : TerRoguelikePacket
    {
        public override PacketType MessageType => PacketType.StageCountSync;
        public static void Send(int toClient = -1, int ignoreClient = -1)
        {
            if (Main.netMode == NetmodeID.SinglePlayer)
                return;

            var packet = NewPacket(PacketType.StageCountSync);
