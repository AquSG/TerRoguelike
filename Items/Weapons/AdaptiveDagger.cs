...
namespace TerRoguelike.Items.Weapons
{
    public class AdaptiveDagger : ModItem, ILocalizedModType
    {
        ...
        public override void UseItemFrame(Player player)
        {
            ...
            if (modPlayer.mouseWorld.X > player.Center.X && modPlayer.swingAnimCompletion <= 0.00001f)
            {
                player.ChangeDir(1);
            }
            else if (modPlayer.mouseWorld.X <= player.Center.X && modPlayer.swingAnimCompletion <= 0.00001f)
            {
                player.ChangeDir(-1);
            }
