...
namespace TerRoguelike.Projectiles
{
    public class MagicFist : ModProjectile, ILocalizedModType
    {
        ...
        public override void AI()
        {
            if (targetIndex != -1)
            {
                if (!Main.npc[(int)targetIndex].active || Main.npc[(int)targetIndex].life <= 0)
                    targetIndex = -1;
            }
            if (Projectile.timeLeft > 6 && targetIndex != -1)
            {
                Projectile.Center = Main.npc[(int)targetIndex].Center + new Vector2(0, -96f);
            }
            else
            {
                ...
                if (targetIndex != -1)
                    ...
                    Projectile.Center += new Vector2(0, 16f);
            }
        ...
        public override bool? CanDamage() => Projectile.timeLeft <= 6 ? (bool?)null : false;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            float opacity = MathHelper.Clamp(MathHelper.Lerp(1f, 0f, (Projectile.timeLeft - 6) / 30f), 0f, 1f);
